...
void NerveStateCtrl::addState(al::NerveStateBase* state, const al::Nerve* nerve, const char* name) {
...
NerveStateCtrl::State* NerveStateCtrl::findStateInfo(const al::Nerve* nerve) {
...
void NerveStateCtrl::tryEndCurrentState() {
    if (mCurrentState) {
        ...
        al::NerveKeeper* keeper = mCurrentState->state->getNerveKeeper();

        if (keeper) {
            al::NerveStateCtrl* ctrl = keeper->getStateCtrl();
