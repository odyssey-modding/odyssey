...
namespace al {
    void resetPosition(al::LiveActor *actor) {
        if (actor->getPoseKeeper()) actor->calcAnim();
        ...
        if (actor->getCollider()) actor->getCollider()->onInvalidate();
        if (actor->getCollisionParts()) al::resetAllCollisionMtx(actor);
        if (!actor->getAudioKeeper()) return;
        if (!actor->getAudioKeeper()->getSeKeeper()) return;
