...
void Rail::init(const al::PlacementInfo& info) {
    isClosed = false;
    al::tryGetArg(&isClosed, info, "IsClosed");
    al::PlacementInfo railPointsInfo;
    al::tryGetPlacementInfoByKey(&railPointsInfo, info, "RailPoints");
    mRailPointsCount = al::getCountPlacementInfo(railPointsInfo);
    if (mRailPointsCount <= 0)
        return;

    mRailPoints = new al::PlacementInfo*[mRailPointsCount];
    for (s32 i = 0; i < mRailPointsCount; i++) {
        mRailPoints[i] = new al::PlacementInfo();
        al::tryGetPlacementInfoByIndex(mRailPoints[i], railPointsInfo, i);
    }

    if (mRailPointsCount == 1) {
        mRailPartCount = 1;
        mRailPart = new RailPart[1];
        al::PlacementInfo partInfo;
        al::tryGetPlacementInfoByIndex(&partInfo, railPointsInfo, 0);
        sead::Vector3f pos = sead::Vector3f::zero;
        al::tryGetRailPointPos(&pos, partInfo);
    ...
    for (s32 i = 0; i < mRailPartCount; i++) {
        al::PlacementInfo startInfo, endInfo;
        al::tryGetPlacementInfoByIndex(&startInfo, railPointsInfo, i);
        al::tryGetPlacementInfoByIndex(&startInfo, railPointsInfo, (i + 1) % mRailPartCount);
        ...
        al::tryGetRailPointPos(&start, startInfo);
        al::getRailPointHandleNext(&startHandle, startInfo);
        al::getRailPointHandlePrev(&endHandle, endInfo);
        al::tryGetRailPointPos(&end, endInfo);
...
f32 Rail::normalizeLength(f32 distance) const {
    if (isClosed) {
        f32 distanceOnRail = al::modf(distance, getTotalLength());
