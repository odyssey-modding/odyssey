...
AreaObj* createAreaObj(const al::ActorInitInfo& actorInitInfo, const char* name) {
    ...
    areaInitInfo.set(actorInitInfo.getPlacementInfo(), actorInitInfo.getStageSwitchDirector(), actorInitInfo.getActorSceneObjHolder());
    al::AreaObj* areaObj = new al::AreaObj(name);
    areaObj->init(areaInitInfo);
    return areaObj;
}

template <typename T>
al::AreaObj* createAreaObjFunction(const char* name) {
    return new T(name);
}
